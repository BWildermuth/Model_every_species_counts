...
# Load data:
data<-read.csv2("raw_data_before_price_Jena.csv", dec = ("."), stringsAsFactors = F)
...
raw_data<-read.csv2("Price_data_jena_temp_updated.csv", dec = ("."), stringsAsFactors = F)
    ...
    # Set up data for analysis
{
  ...
  levels_k <- unique(data4$species_mixture_output.k.) # PSR levels
  ...
  for (l in seq_along(var_names)) {
    ...
    results <- with_progress({
      ...
      future_map(1:n_iter, function(i) {
        ...
        #model
        mod <- lmer(
          response ~ scale(diff) * scale(species_mixture_output.k.) +
        ...
        trends_obj <- suppressWarnings(
          emtrends(mod,
                   ~ species_mixture_output.k.,
                   var  = "diff",
                   at   = list(species_mixture_output.k. = levels_k),
        ...
        list(
          ...
          pred1  = suppressWarnings(
            ggemmeans(mod, c("diff", "species_mixture_output.k."))),
    ...
    trend_summary <- trend_all %>%
      group_by(species_mixture_output.k.) %>%
      summarise(
