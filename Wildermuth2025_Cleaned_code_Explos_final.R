...
# Load data:
data<-read.csv2("raw_data_cleaned_Explos.csv", dec = ("."), stringsAsFactors = F)


# define land-use intensity (LUI) categories
{
  # extensive
...
raw_data<-read.csv2("Price_data_explos_clean.csv", dec = ("."), stringsAsFactors = F)
  ...
  # Set up data for analysis
{
  ...
  data3 <- data3[data3$temp_diff != 10, ] # remove time spans (temp_diff) with only one replicate
  ...
  levels_k <- c(0.5, 1, 1.5, 2, 2.5, 3, 3.5) # LUI levels
  ...
  sample_plot_pairs <- function(df_plot, base_yrs = c(2008, 2009, 2010, 2011, 2012)) { # restricted moving average
    df_plot <- df_plot %>%
      mutate(
  ...
  for (l in seq_along(var_names)) {
    ...
    results <- with_progress({
      ...
      future_map(1:n_iter, function(i) {
        ...
        #model
        mod <- lmer(
          response ~ scale(temp_diff) * scale(LUI_mean) +
        ...
        #trends per LUI level
        trends_obj <- suppressWarnings(
          emtrends(mod,
                   ~ LUI_mean,
                   var  = "temp_diff",
                   at   = list(LUI_mean = levels_k),
        ...
        list(
          ...
          pred1  = suppressWarnings(
            ggemmeans(mod, c("temp_diff", "LUI_mean [levels_k]"))),
    ...
    #store and summarize trends per LUI level
    ...
    #store and summarize predictions per LUI level
