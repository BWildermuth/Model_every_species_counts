...
# Load data:
data<-read.csv2("raw_data_before_price_Explos.csv", dec = ("."), stringsAsFactors = F)
  ...
  # Set up data for analysis
{
  ...
  for (l in seq_along(var_names)) {
    data4$response <- data4[[var_names[l]]]
    ...
    results <- with_progress({
      ...
      future_map(1:n_iter, function(i) {
        ...
        #model
        mod <- lmer(
          response ~ scale(diff) * scale(LUI_numeric) +
        ...
        list(
          ...
          pred1  = suppressWarnings(
            ggemmeans(mod, c("diff", "LUI_numeric [levels_k]"))),
          pred2  = suppressWarnings(ggemmeans(mod, "diff"))
        )
    ...
    trend_col <- intersect(names(trend_all), c("trend", "diff.trend"))[1]
    
    trend_summary <- trend_all %>%
      group_by(LUI_numeric) %>%
      summarise(
